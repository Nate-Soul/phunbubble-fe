...
const Navbar = () => {
  ...
  return (
    <nav className="shadow flex items-center justify-between text-sm h-16 px-8 sticky top-0 bg-white dark:bg-gray-900 z-40 max-w-full">
      <Link href="/">
        <Image src="/phunbubble.svg" width={50} height={70} />
      </Link>
      <form className="flex items-center md:border border-gray-200 p-1 px-3 rounded-3xl hover:border-secondary-100">
        ...
        <input
          ...
          id="search"
          placeholder="Search..."
          className="w-0 md:w-40 lg:w-96 outline-none p-1 border-0"
        />
      </form>
      <ul className="flex items-center gap-6">
        ...
        <li className="relative">
          <Image
            src={currentUser.length ? currentUser.profilePic : Avatar}
            className="h-6 w-6 rounded-full object-cover cursor-pointer"
            alt={currentUser.length ? currentUser.name : "avatar icon"}
            onClick={toggleUserMenu}
          />
          {userMenu && (
            <ul className="absolute bg-gray-200 p-3 right-0 top-6 mt-2 rounded w-max flex flex-col gap-2 triangle-up">
